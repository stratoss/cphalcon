...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, _0, eventType, 0 TSRMLS_CC))) {
		...
		if (zephir_is_true(_1$$4)) {
			...
			ZVAL_LONG(_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, priorityQueue, "setextractflags", NULL, 187, _2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, priorityQueue, "insert", NULL, 188, handler, _3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, _0, eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, newPriorityQueue, "setextractflags", NULL, 187, _1$$5);
			zephir_check_call_status();
			...
			while (1) {
				...
				if (!ZEPHIR_IS_IDENTICAL(_5$$6, handler)) {
					...
					ZEPHIR_CALL_METHOD(NULL, newPriorityQueue, "insert", &_8, 188, _6$$7, _7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), eventType, newPriorityQueue TSRMLS_CC);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 189, handler, priorityQueue, ZEPHIR_GLOBAL(global_true));
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, enablePriorities) {
	...
	if (enablePriorities) {
		zephir_update_property_this(this_ptr, SL("_enablePriorities"), ZEPHIR_GLOBAL(global_true) TSRMLS_CC);
	} else {
		zephir_update_property_this(this_ptr, SL("_enablePriorities"), ZEPHIR_GLOBAL(global_false) TSRMLS_CC);
	}
...
PHP_METHOD(Phalcon_Events_Manager, collectResponses) {
	...
	if (collect) {
		zephir_update_property_this(this_ptr, SL("_collect"), ZEPHIR_GLOBAL(global_true) TSRMLS_CC);
	} else {
		zephir_update_property_this(this_ptr, SL("_collect"), ZEPHIR_GLOBAL(global_false) TSRMLS_CC);
	}
...
PHP_METHOD(Phalcon_Events_Manager, fireQueue) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *queue, *event, *status = NULL, *arguments = NULL, *eventName = NULL, *data = NULL, *iterator = NULL, *source = NULL, *handler = NULL, *_4 = NULL, *_5, *_0$$5, *_1$$5, _2$$5, *_3$$5 = NULL, *_6$$8 = NULL, *_9$$14 = NULL, *_11$$19 = NULL, **_14$$21, *_15$$27 = NULL, *_16$$32 = NULL;
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue TSRMLS_CC))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 190, &_2$$5, _1$$5);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, _0$$5, "__construct", NULL, 9, _3$$5);
				zephir_check_call_status();
				zephir_throw_exception_debug(_0$$5, "phalcon/events/manager.zep", 213 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} else {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_4, event, "getcancelable", NULL, 0);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(queue) == IS_OBJECT) {
		...
		while (1) {
			...
			if (Z_TYPE_P(handler) == IS_OBJECT) {
				if (zephir_instance_of_ev(handler, zend_ce_closure TSRMLS_CC)) {
					...
					if (cancelable) {
						ZEPHIR_CALL_METHOD(&_9$$14, event, "isstopped", &_10, 0);
						zephir_check_call_status();
					...
					if ((zephir_method_exists(handler, eventName TSRMLS_CC)  == SUCCESS)) {
						...
						if (cancelable) {
							ZEPHIR_CALL_METHOD(&_11$$19, event, "isstopped", &_10, 0);
							zephir_check_call_status();
		...
		zephir_is_iterable(queue, &_13$$21, &_12$$21, 0, 0, "phalcon/events/manager.zep", 364);
		for (
		  ; zephir_hash_get_current_data_ex(_13$$21, (void**) &_14$$21, &_12$$21) == SUCCESS
		  ; zephir_hash_move_forward_ex(_13$$21, &_12$$21)
		) {
			ZEPHIR_GET_HVALUE(handler, _14$$21);
			if (Z_TYPE_P(handler) == IS_OBJECT) {
				if (zephir_instance_of_ev(handler, zend_ce_closure TSRMLS_CC)) {
					if (Z_TYPE_P(arguments) == IS_NULL) {
						ZEPHIR_INIT_NVAR(arguments);
					...
					if (cancelable) {
						ZEPHIR_CALL_METHOD(&_15$$27, event, "isstopped", &_10, 0);
						zephir_check_call_status();
					...
					if ((zephir_method_exists(handler, eventName TSRMLS_CC)  == SUCCESS)) {
						...
						if (cancelable) {
							ZEPHIR_CALL_METHOD(&_16$$32, event, "isstopped", &_10, 0);
							zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	if (!(zephir_memnstr_str(eventType, SL(":"), "phalcon/events/manager.zep", 392))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_0$$4, "phalcon/events/manager.zep", 393 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, events, type, 0 TSRMLS_CC)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, event, "__construct", &_5, 191, eventName, source, data, _4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_6, 192, fireEvents, event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, events, eventType, 0 TSRMLS_CC)) {
		...
		if (_7$$8) {
			if (Z_TYPE_P(event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, event, "__construct", &_5, 191, eventName, source, data, _8$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_6, 192, fireEvents, event);
			zephir_check_call_status();
