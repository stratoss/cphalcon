...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "extension_loaded", NULL, 131, &_0);
	...
	if (!ZEPHIR_IS_STRING_IDENTICAL(callbacks, "")) {
		...
		ZEPHIR_CALL_FUNCTION(&yamlConfig, "yaml_parse_file", &_4, 132, filePath, _2$$4, _3$$4, callbacks);
		ZEPHIR_UNREF(_3$$4);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&yamlConfig, "yaml_parse_file", &_4, 132, filePath);
		zephir_check_call_status();
	}
