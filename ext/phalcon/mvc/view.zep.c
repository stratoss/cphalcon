...
PHP_METHOD(Phalcon_Mvc_View, _loadTemplateEngines) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(engines)) {
		...
		if (Z_TYPE_P(registeredEngines) != IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(NULL, _1$$4, "__construct", NULL, 367, this_ptr, dependencyInjector);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View, render) {
	...
	if (!ZEPHIR_IS_FALSE(_0)) {
		ZEPHIR_CALL_FUNCTION(&_1$$3, "ob_get_contents", &_2, 122);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_9, "ob_get_contents", &_2, 122);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View, getPartial) {
	...
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("ob_get_clean", NULL, 286);
	zephir_check_call_status();
	RETURN_MM();
