...
PHP_METHOD(Phalcon_Mvc_Micro, mount) {
	...
	if (Z_TYPE_P(handlers) == IS_ARRAY) {
		...
		if (zephir_is_true(_0$$5)) {
			...
			ZEPHIR_CALL_METHOD(NULL, lazyHandler, "__construct", NULL, 299, mainHandler);
			zephir_check_call_status();
		} else {
...
PHP_METHOD(Phalcon_Mvc_Micro, setService) {
	...
	if (Z_TYPE_P(dependencyInjector) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, dependencyInjector, "__construct", NULL, 177);
		zephir_check_call_status();
		zephir_update_property_this(this_ptr, SL("_dependencyInjector"), dependencyInjector TSRMLS_CC);
	}
	ZEPHIR_INIT_VAR(_0);
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "set", NULL, 300, serviceName, definition, _0);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Micro, hasService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "has", NULL, 301, serviceName);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Micro, getService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "get", NULL, 302, serviceName);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Mvc_Micro, getSharedService) {
	...
	ZEPHIR_RETURN_CALL_METHOD(dependencyInjector, "getshared", NULL, 303, serviceName);
	zephir_check_call_status();
	RETURN_MM();
