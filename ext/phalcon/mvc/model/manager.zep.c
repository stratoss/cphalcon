...
PHP_METHOD(Phalcon_Mvc_Model_Manager, getRelationRecords) {
	...
	if (zephir_is_true(_0)) {
		...
		ZEPHIR_CALL_METHOD(&_5$$3, this_ptr, "_mergefindparameters", &_6, 316, extraParameters, parameters);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&findArguments, this_ptr, "_mergefindparameters", &_6, 316, findParams, parameters);
	zephir_check_call_status();
	if (Z_TYPE_P(extraParameters) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&findParams, this_ptr, "_mergefindparameters", &_6, 316, findArguments, extraParameters);
		zephir_check_call_status();
	} else {
