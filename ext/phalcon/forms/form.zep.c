		...
		if (Z_TYPE_P(validators) == IS_ARRAY) {
			if (zephir_fast_count_int(validators TSRMLS_CC)) {
				...
				if (Z_TYPE_P(validation) == IS_OBJECT) {
					...
					ZEPHIR_CALL_METHOD(NULL, validation, "__construct", &_11, 213, preparedValidators);
					zephir_check_call_status();
				}
				ZEPHIR_CALL_METHOD(&filters, element, "getfilters", NULL, 0);
				zephir_check_call_status();
				if (Z_TYPE_P(filters) == IS_ARRAY) {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(NULL, validation, "setfilters", &_13, 214, _12$$16, filters);
					zephir_check_call_status();
				}
				ZEPHIR_CALL_METHOD(&elementMessages, validation, "validate", &_14, 215, data, entity);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(messages) == IS_ARRAY) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_2$$5, &_1$$5)
			...
			ZEPHIR_CALL_METHOD(NULL, group, "appendmessages", &_4, 216, elementMessages);
			zephir_check_call_status();
		}
	}
	RETURN_CCTOR(group);
...
PHP_METHOD(Phalcon_Forms_Form, rewind) {
	...
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("_elements"), PH_NOISY_CC);
	ZEPHIR_CALL_FUNCTION(&_1, "array_values", NULL, 217, _0);
	zephir_check_call_status();
