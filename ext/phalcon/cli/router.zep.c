...
PHP_METHOD(Phalcon_Cli_Router, __construct) {
	...
	if (defaultRoutes == 1) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0$$3, "__construct", &_3, 128, _2$$3, _1$$3);
		...
		ZEPHIR_CALL_METHOD(NULL, _2$$3, "__construct", &_3, 128, _5$$3, _4$$3);
		...
		if (zephir_array_isset_string_fetch(&params, parts, SS("params"), 0 TSRMLS_CC)) {
			if (Z_TYPE_P(params) != IS_ARRAY) {
				...
				if (zephir_is_true(strParams)) {
					ZEPHIR_CALL_CE_STATIC(&_19$$32, phalcon_cli_router_route_ce, "getdelimiter", &_20, 126);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cli_Router, add) {
	...
	ZEPHIR_CALL_METHOD(NULL, route, "__construct", NULL, 128, pattern, paths);
	zephir_check_call_status();
