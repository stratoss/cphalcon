...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	ZEPHIR_CALL_METHOD(&valid, this_ptr, "verifybyluhnalgorithm", NULL, 444, value);
	zephir_check_call_status();
	if (!(zephir_is_true(valid))) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0$$3, "__construct", NULL, 440, _2$$3, field, _3$$3);
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	ZVAL_STRING(hash, "", 1);
	ZEPHIR_CALL_FUNCTION(&_4, "array_reverse", NULL, 445, digits);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 446, _9);
	zephir_check_call_status();
