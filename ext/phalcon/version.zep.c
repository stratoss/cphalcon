...
PHP_METHOD(Phalcon_Version, get) {
	...
	ZEPHIR_CALL_STATIC(&suffix, "_getspecial", &_0, 449, special);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Version, getId) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "sprintf", &_2, 190, &_0, medium);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_0);
	ZVAL_STRING(&_0, "%02s", 0);
	ZEPHIR_CALL_FUNCTION(&_3, "sprintf", &_2, 190, &_0, minor);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Version, getPart) {
	...
	do {
		...
		if (part == 3) {
			...
			ZEPHIR_CALL_STATIC(&result, "_getspecial", &_0, 449, _1$$4);
			zephir_check_call_status();
			break;
		}
