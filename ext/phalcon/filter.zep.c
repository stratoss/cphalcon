...
PHP_METHOD(Phalcon_Filter, sanitize) {
	...
	if (Z_TYPE_P(filters) == IS_ARRAY) {
		if (Z_TYPE_P(value) != IS_NULL) {
			zephir_is_iterable(filters, &_1$$4, &_0$$4, 0, 0, "phalcon/filter.zep", 107);
			for (
			  ...
			  ; zephir_hash_move_forward_ex(_1$$4, &_0$$4)
				...
				if (_3$$5) {
					...
					zephir_is_iterable(value, &_5$$6, &_4$$6, 0, 0, "phalcon/filter.zep", 102);
					for (
	...
	if (_10) {
		...
		zephir_is_iterable(value, &_12$$9, &_11$$9, 0, 0, "phalcon/filter.zep", 119);
		for (
...
PHP_METHOD(Phalcon_Filter, _sanitize) {
	...
	do {
		if (ZEPHIR_IS_STRING(filter, "email")) {
			...
			ZEPHIR_CALL_FUNCTION(&_4$$5, "constant", NULL, 193, &_3$$5);
			zephir_check_call_status();
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 194, value, _4$$5);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "int")) {
			...
			ZVAL_LONG(&_6$$6, 519);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 194, value, &_6$$6);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "int!")) {
		...
		if (ZEPHIR_IS_STRING(filter, "string")) {
			...
			ZVAL_LONG(&_8$$9, 513);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 194, value, &_8$$9);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "float")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 194, value, &_10$$10, _9$$10);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "float!")) {
