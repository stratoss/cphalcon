...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, __construct) {
	...
	if (zephir_is_true(name)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "openlog", NULL, 292, name, option, facility);
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, logInternal) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 293, _3, _4);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, close) {
	...
	if (zephir_is_true(_0)) {
		ZEPHIR_CALL_FUNCTION(NULL, "closelog", NULL, 294);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
