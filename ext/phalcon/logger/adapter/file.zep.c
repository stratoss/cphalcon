...
PHP_METHOD(Phalcon_Logger_Adapter_File, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&handler, "fopen", NULL, 287, name, mode);
	...
	if (Z_TYPE_P(handler) != IS_RESOURCE) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0$$7, "__construct", NULL, 9, _1$$7);
		zephir_check_call_status();
		zephir_throw_exception_debug(_0$$7, "phalcon/logger/adapter/file.zep", 88 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Logger_Adapter_File, getFormatter) {
	...
	if (Z_TYPE_P(_0) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, _1$$3, "__construct", NULL, 291);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Logger_Adapter_File, __wakeup) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "fopen", NULL, 287, path, mode);
	zephir_check_call_status();
